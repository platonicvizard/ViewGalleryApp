...
{#if items.length > 0}
  <div
    bind:this={strip}
    class="flex gap-2 px-3 py-2 overflow-x-auto bg-surface/60 border-t border-white/5"
  >
    {#each items as item (item.path)}
      <button
        data-path={item.path}
        class="relative shrink-0 w-16 h-16 rounded-md overflow-hidden border-2 transition {item.path ===
        gallery.currentPath
          ? 'border-accent'
